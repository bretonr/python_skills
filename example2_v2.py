...
for i in np.arange(nbursts):
    ...
    ## Calculate the time delay at each frequency for a DM of 1
    t_at_freqs = t_at_freqs_DM1*DM

    ## Calculate the flux amplitude at each frequency
    flux_at_freqs = Spectrum(amp_burst, bottom_freq, alpha, freqs)

    ## For each channel, draw a noise level from a lognormal distribution,
    ## then generate white noise according to this noise level.
    noise_std_per_channel = rng.lognormal(mean=avg_chan_noise, sigma=std_chan_noise, size=nchannels)
    noise_arr_tmp = rng.normal(loc=0., scale=1.0, size=(nchannels, nsamp))*noise_std_per_channel[:,None]

    ## Generate the model
    model_arr_tmp = Calc_model(flux_at_freqs[:,None], tstamp[None,:], t_burst, t_at_freqs[:,None], duration_burst)
